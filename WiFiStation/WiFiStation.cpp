  ...
  , m_onStop(std::move(onStop))
{
  ...
  ESP_ERROR_CHECK(
    esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &WiFiStation::wifiEventHandler, this, &m_eventWiFi)
  );
  ESP_ERROR_CHECK(
    esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &WiFiStation::ipEventHandler, this, &m_eventIp)
  );
}
